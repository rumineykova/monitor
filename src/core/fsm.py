...
class FSM:
    ...
    def get_transition_from_memory_old(self, input_symbol, state):
        ...
        print transitionStacks
        for stack in transitionStacks:
            if (stack and (stack[0] == input_symbol)):
                stackToPop = stack
                break 
            
        if stackToPop is not None:
                stackToPop.popleft()
                if not(stackToPop):
    ...
    def get_transition_old (self, input_symbol, state):
        ...
        if ((input_symbol is not self.EMPTY_TRANSITION) and has_memory):
            return self.get_transition_from_memory_old(input_symbol, state)
