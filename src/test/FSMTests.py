...
class TestFSM(unittest.TestCase):
    def get_test_fsm(self):
        # build test data
        testData = FSM('1_1')
        testData.state_transitions = {('a', '1_1'):(None, '1_2')}
        return testData
    ...
    def test_nested_transition_for_first_time(self):
        ...
        fsm.add_nested_transition('a', 1, '1_1', None, '1_2')
    ...
    def test_get_transition_from_memory_when_there_is_a_match(self):
        ...
        (_, next_state) = fsm.get_transition('a', 1)
    ...
    def test_get_normal_transition_when_there_is_a_match(self):
        ...
        (_, next_state) = fsm.get_transition('b', 1)
    ...
    def test_get_normal_transition_when_there_is_no_match_but_such_transition_exist(self):
        ...
        #fsm.state_transitions = {('b', 1):(None, 2)}
        first_fsm = FSM('1_1')
        first_fsm.state_transitions = {('a', '1_1'):(None, '1_2'), (fsm.END_PAR_TRANSITION, '1_2'): (None, 2)}
        fsm.memory = {2: [first_fsm]}
        (action, next_state) = fsm.get_transition( 'a', 2)
